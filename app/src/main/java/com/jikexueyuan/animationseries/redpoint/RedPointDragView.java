...
class RedPointDragView extends View {
    private static final String TAG = RedPointDragView.class.getSimpleName();
    ...
    private int mStartX;
    private int mStartY;
    private RedPointExplosionAnimator mExplosionAnimator;
    ...
    public RedPointDragView(Context context, RedPointViewHelper redPointViewHelper) {
        super(context);
        mWindowManager = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
        mRedPointViewHelper = redPointViewHelper;
        initBadgePaint();
        initLayoutParams();
        initStick();
    }
    ...
    private void initBadgePaint() {
        ...
        mRedPointPaint.setTextAlign(Paint.Align.CENTER);
        mRedPointPaint.setTextSize(mRedPointViewHelper.getRedPointTextSize());
    }
    ...
    private void initStick() {
        mMaxDragRadius = RedPointViewUtil.dp2px(getContext(), 10);
        mDragStickRadiusDifference = RedPointViewUtil.dp2px(getContext(), 1);
    }
    ...
    protected void onDraw(Canvas canvas) {
        try {
            if (mExplosionAnimator == null) {
                if (mRedPointViewHelper.isShowDrawable()) {
                    if (mRedPointViewHelper.getRedPointBgColor() == Color.RED) {
                        mRedPointPaint.setColor(mRedPointViewHelper.getBitmap().getPixel(mRedPointViewHelper.getBitmap().getWidth() / 2, mRedPointViewHelper.getBitmap().getHeight() / 2));
                    } else {
                        mRedPointPaint.setColor(mRedPointViewHelper.getRedPointBgColor());
                    }
                    drawStick(canvas);
    ...
    private void drawDrawableBadge(Canvas canvas) {
        canvas.drawBitmap(mRedPointViewHelper.getBitmap(), mStartX, mStartY, mRedPointPaint);
    }
    ...
    private void drawTextBadge(Canvas canvas) {
        ...
        mRedPointPaint.setColor(mRedPointViewHelper.getRedPointBgColor());
        // 绘制徽章背景
        canvas.drawRoundRect(new RectF(mStartX, mStartY, mStartX + mRedPointViewHelper.getRedPointRectF().width(), mStartY + mRedPointViewHelper.getRedPointRectF().height()), mRedPointViewHelper.getRedPointRectF().height() / 2, mRedPointViewHelper.getRedPointRectF().height() / 2, mRedPointPaint);

        // 设置徽章文本颜色
        mRedPointPaint.setColor(mRedPointViewHelper.getRedPointTextColor());
        float x = mStartX + mRedPointViewHelper.getRedPointRectF().width() / 2;
        // 注意：绘制文本时的y是指文本底部，而不是文本的中间
        float y = mStartY + mRedPointViewHelper.getRedPointRectF().height() - mRedPointViewHelper.getRedPointPadding();
        // 绘制徽章文本
        String badgeText = mRedPointViewHelper.getRedPointText() == null ? "" : mRedPointViewHelper.getRedPointText();
        canvas.drawText(badgeText, x, y, mRedPointPaint);
    }
    ...
    private void drawStick(Canvas canvas) {
        ...
        // 通过几何图形工具获取交点坐标
        mDragPoints = RedPointViewUtil.getIntersectionPoints(mDragCenter, mDragRadius, lineK);
        mStickPoints = RedPointViewUtil.getIntersectionPoints(mStickCenter, currentStickRadius, lineK);

        // 3. 获取控制点坐标
        mControlPoint = RedPointViewUtil.getMiddlePoint(mDragCenter, mStickCenter);

        // 保存画布状态
        canvas.save();
        canvas.translate(0, -RedPointViewUtil.getStatusBarHeight(mRedPointViewHelper.getRootView()));
        ...
        if (!mIsDragDisappear) {
            if (!mDismissAble) {
                ...
                path.close();
                canvas.drawPath(path, mRedPointPaint);

                // 2. 画固定圆
                canvas.drawCircle(mStickCenter.x, mStickCenter.y, currentStickRadius, mRedPointPaint);
            }

            // 1. 画拖拽圆
            canvas.drawCircle(mDragCenter.x, mDragCenter.y, mDragRadius, mRedPointPaint);
        }
    ...
    private float getCurrentStickRadius() {
         ...
         */
        float distance = RedPointViewUtil.getDistanceBetween2Points(mDragCenter, mStickCenter);
        distance = Math.min(distance, mDismissThreshold);
        float percent = distance / mDismissThreshold;
        return RedPointViewUtil.evaluate(percent, mStickRadius, mStickRadius * 0.2f);
    }
    ...
    private void handleActionDown(MotionEvent event) {
        if (mExplosionAnimator == null && getParent() == null) {
            mDragRadius = Math.min(mRedPointViewHelper.getRedPointRectF().width() / 2, mMaxDragRadius);
    ...
    private void handleActionMove(MotionEvent event) {
        if (mExplosionAnimator == null && getParent() != null) {
            ...
            // 处理断开事件
            if (RedPointViewUtil.getDistanceBetween2Points(mDragCenter, mStickCenter) > mDismissThreshold) {
                mDismissAble = true;
                postInvalidate();
            } else if (mRedPointViewHelper.isResumeTravel()) {
                mDismissAble = false;
                postInvalidate();
            }
        }
        ...
        if (mDismissAble) {
            ...
            if (RedPointViewUtil.getDistanceBetween2Points(mDragCenter, mStickCenter) > mDismissThreshold) {
                ...
                try {
                    ...
                    removeSelf();
                    mRedPointViewHelper.endDragWithDismiss();
                }
            } else {
                // 现在没有超出范围,放回去
                removeSelf();
                mRedPointViewHelper.endDragWithoutDismiss();
            }
        } else {
            //	拖拽点没超出过范围,弹回去
            try {
    ...
    private void startSpringAnim() {
        ...
        springAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator mAnim) {
                ...
                PointF p = RedPointViewUtil.getPointByPercent(startReleaseDragCenter, mStickCenter, percent);
        ...
        springAnim.setDuration(RedPointExplosionAnimator.ANIM_DURATION / 2);
        springAnim.start();
    }

    private void startDismissAnim(int newX, int newY) {
        int badgeWidth = (int) mRedPointViewHelper.getRedPointRectF().width();
        int badgeHeight = (int) mRedPointViewHelper.getRedPointRectF().height();
        Rect rect = new Rect(newX - badgeWidth / 2, newY - badgeHeight / 2, newX + badgeWidth / 2, newY + badgeHeight / 2);

        Bitmap badgeBitmap = RedPointViewUtil.createBitmapSafely(this, rect, 1);
        if (badgeBitmap == null) {
            removeSelf();
            mRedPointViewHelper.endDragWithDismiss();
            return;
        ...
        mExplosionAnimator = new RedPointExplosionAnimator(this, rect, badgeBitmap);
    ...
    private int getNewStartY(float rawY) {
        ...
        int newStartY = (int) rawY - badgeHeight / 2 - RedPointViewUtil.getStatusBarHeight(mRedPointViewHelper.getRootView());
